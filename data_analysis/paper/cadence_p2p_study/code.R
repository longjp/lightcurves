...
## P2p scatter : median of the absolute values of the differences
## between successive magnitudes in the raw light curve normalized
## by the MAD around the median.
DubathFeature = function(fluxes){
  num = median(abs(fluxes[2:length(fluxes)] - fluxes[1:(length(fluxes) -1)]))
  den = median(abs(fluxes - median(fluxes)))
  return( num / den )
}

GenerateMeasurements = function(lc_parms,cadence=1,number_flux=200){
  mag = 1
  period = runif(1,min=lc_parms[1]-lc_parms[2]/2,max=lc_parms[1]+lc_parms[2]/2)
...
GenerateAndCompute = function(lc_parms,cadence=1,number_flux=200,number_reps=1){
  vals = rep(0,number_reps)
  for(i in 1:length(vals)){
    fluxes = GenerateMeasurements(lc_parms,cadence,number_flux)[[2]]
