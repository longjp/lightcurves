...
# now do euclidean distance
  for(i in 1:dim(neighbors)[3]){
    ...
    ordered.train.ids = data1train.temp$features.source_id
    data1train.temp = subset(data1train.temp,select=imp.variables)
    data1train.temp = as.matrix(data1train.temp)
  
    ## find out what is happening
    ## print("ordered train ids information:")
    ## print(length(ordered.train.ids))
    ## print(length(unique(ordered.train.ids)))
    ...
    for(j in 1:(dim(neighbors)[1])){
      print(j)
      current.vec = data1train.temp[j,]
      distances = apply(data1train.temp,
        1,function(z) { sum((z - current.vec)^2)})
      neighbors[j,,i] = ordered.train.ids[
                 order(distances,decreasing=TRUE) < (nNN + .5)]
    }
    
    ## print("number of test rows")
    ## print(nrow(data1test.temp))
    ## print("number of train rows")
  ## print(nrow(data1train.temp))
  ## print("the number of columns in train is")
  ## print(ncol(data1train.temp))
    ## #print(head(data1train.temp))
    ## #print(head(data1test.temp))
...
# the
id.association = data1$sources.original_source_id
names(id.association) = data1$features.source_id
length(id.association)

classification = function(ids){
  ids = id.association[as.character(ids)]
